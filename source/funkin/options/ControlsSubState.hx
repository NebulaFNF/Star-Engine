...
class ControlsSubState extends MusicBeatSubstate {
	...
	override function update(elapsed:Float) {
		if(!rebindingKey) {
			...
			if (keyPressed > -1) {
				...
				if(keysArray[opposite] == keysArray[1 - opposite]) keysArray[opposite] = NONE;
	...
	function getInputTextNum() {
		var num:Int = 0;
		for (i in 0...curSelected) if(optionShit[i].length > 1) num++;
		return num;
	}
	...
	function changeAlt() {
		...
		for (i in 0...grpInputsAlt.length) {
			if(grpInputsAlt[i].sprTracker == grpOptions.members[curSelected]) {
				...
				if(curAlt) grpInputsAlt[i].alpha = 1;
				break;
			}
		}
		FlxG.sound.play(Paths.sound('scrollMenu'));
	...
	function reloadKeys() {
		...
		for (i in 0...grpOptions.length) if(!unselectableCheck(i, true)) addBindTexts(grpOptions.members[i], i);
		...
		for (item in grpOptions.members) {
			...
			if(!unselectableCheck(bullShit-1)) {
				...
				if (item.targetY == 0) {
					item.alpha = 1;
					if(curAlt)
						for (i in 0...grpInputsAlt.length) if(grpInputsAlt[i].sprTracker == item) grpInputsAlt[i].alpha = 1;
					else
						for (i in 0...grpInputs.length) if(grpInputs[i].sprTracker == item) grpInputs[i].alpha = 1;
				}
			}
		}
	}
