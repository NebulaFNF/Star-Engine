...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		funkin.play.PlayState.isStoryMode = false;
	...
	function weekIsLocked(name:String):Bool {
		...
		return (!leWeek.startUnlocked && leWeek.weekBefore.length > 0 && (!funkin.ui.menu.StoryMenuState.weekCompleted.exists(leWeek.weekBefore) || !funkin.ui.menu.StoryMenuState.weekCompleted.get(leWeek.weekBefore)));
	}
	...
	override function update(elapsed:Float)
	{
		...
		{
			persistentUpdate = false;
			openSubState(new funkin.ui.play.GameplayChangersSubstate());
		}
		...
		else if (accepted)
		{
			...
			funkin.play.PlayState.SONG = Song.loadFromJson(poop, songLowercase);
			funkin.play.PlayState.isStoryMode = false;
			funkin.play.PlayState.storyDifficulty = curDifficulty;

			trace('CURRENT WEEK: ' + WeekData.getWeekFileName());
			if(colorTween != null) colorTween.cancel();

			if (FlxG.keys.pressed.SHIFT) LoadingState.loadAndSwitchState(new funkin.ui.editors.ChartingState());
			else LoadingState.loadAndSwitchState(new funkin.play.PlayState());
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		funkin.play.PlayState.storyWeek = songs[curSelected].week;
