...
import funkin.play.characters.Character.CharacterFile;
...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Camera Twist', "Makes the camera spin! or twist i guess\nValue 1: Twist intensity\nValue 2: Twist intensity 2"],
		...
		//['Crash the Computer', "put the event in a certain place and boom"]
	];
	...
	**/
	...
	override function create()
	{
		if (funkin.play.PlayState.SONG != null)
			_song = funkin.play.PlayState.SONG;
		else
		{
			...
			addSection();
			funkin.play.PlayState.SONG = _song;
		}
		...
		for (i in 0...tipTextArray.length) {
		    ...
		    theThingImage.x = (UI_box.x + 22);
		    theThingImage.scrollFactor.set();
			var tipText:FlxText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 8, 0, tipTextArray[i], 16);
			tipText.y += i * 12;
			theThingImage.y = tipText.y;
			tipText.setFormat(Paths.font("vcr.ttf"), 14, FlxColor.BLACK, LEFT);
	...
	function addSongUI():Void
	{
		...
		{
			funkin.play.PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
		...
		var skin = funkin.play.PlayState.SONG.arrowSkin;
		if(skin == null) skin = '';
	...
	function addNoteStackingUI():Void
	{
			...
			tab_group_stacking.name = 'Spamming';

			check_stackActive = new FlxUICheckBox(10, 10, null, null, "Enable Spam Mode", 100);
			...
			var dupeNotesButton:FlxButton = new FlxButton(10, stepperDuplicateAmount.y + 20, "Duplicate Notes", function()
			{
				...
				_song.notes[curSec].sectionNotes.length <= 30000 ? updateGrid() : changeSection(curSec + 1); //if there's now more than 30,000 notes in the same section then uh.. change to the next section so you don't suffer a crash
			});
	...
	function loadSong():Void
	{
		...
		var bruh:String = (funkin.play.PlayState.SONG.specialAudioName.length > 1 ? funkin.play.PlayState.SONG.specialAudioName : CoolUtil.difficultyString()).toLowerCase();
		var file:Dynamic = Paths.voices(currentSongName, bruh);
		vocals = new FlxSound();
	...
	function generateSong() {
		var bruh:String = (funkin.play.PlayState.SONG.specialAudioName.length > 1 ? funkin.play.PlayState.SONG.specialAudioName : CoolUtil.difficultyString()).toLowerCase();
		FlxG.sound.playMusic(Paths.inst(currentSongName, bruh), 0.6/*, false*/);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new funkin.ui.editors.EditorPlayState(sectionStartTime()));
			}
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				funkin.play.PlayState.chartingMode = false;
				MusicBeatState.switchState(new funkin.ui.editors.MasterEditorMenu());
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 4;
						...
						strumLineNotes.members[noteDataToCheck].confirmHoldTimer = (note.sustainLength / 1000) + 0.15;
					if(!playedSound[data]) {
	...
	*/
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		var json:funkin.play.characters.Character.CharacterFile = cast Json.parse(rawJson);
	...
	function loadJson(song:String):Void
	{
		...
		if (CoolUtil.difficulties[funkin.play.PlayState.storyDifficulty] != CoolUtil.defaultDifficulty) {
			if(CoolUtil.difficulties[funkin.play.PlayState.storyDifficulty] == null){
				funkin.play.PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
			}else{
				funkin.play.PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + CoolUtil.difficulties[funkin.play.PlayState.storyDifficulty], song.toLowerCase());
			}
		}else{
		funkin.play.PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
		}
