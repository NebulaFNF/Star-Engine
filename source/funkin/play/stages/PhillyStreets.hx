...
import funkin.play.note.Note.PreloadedChartNote;
import funkin.play.note.Note;
import funkin.play.stages.BaseStage;
import funkin.play.stages.objects.ABotSpeaker;
import funkin.play.stages.objects.SpraycanAtlasSprite;
	...
	function videoCutscene(?videoName:String = null)
	{
		...
		if (!videoEnded && videoName != null)
		{
			...
			if (shit == null) // returns an VideoSprite, if it returns null then something went wrong
			{
				videoEnded = true;
				videoCutscene();
				FlxG.log.warn('Video ended early! Was the video not found?X');
			}
