...
import funkin.play.bg.BackgroundDancer;
import funkin.play.bg.BackgroundGirls;
import funkin.play.bg.TankmenBG;
import funkin.play.characters.Boyfriend;
import funkin.play.characters.Character;
import funkin.play.cutscene.CutsceneHandler;
import funkin.play.dialogue.DialogueBox;
import funkin.play.dialogue.DialogueBoxPsych;
...
import funkin.song.*;
import funkin.song.Conductor.Rating;
import funkin.song.Section.SwagSection;
import funkin.song.Song.SwagSong;
import funkin.ui.editors.CharacterEditorState;
import funkin.ui.editors.ChartingState;
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Man', 0.6], //From 50% to 59%
		['Eh', 0.69], //From 60% to 68%
		['Cool', 0.7], //69%
		['Epic', 0.8], //From 70% to 79%
	...
	//public var devBuildTxt:FlxText;
	...
	override public function create()
	{
		...
		funkin.ui.play.pause.PauseSubState.songName = null; //Reset to default
		...
		funkin.ui.transition.CustomFadeTransition.nextCamera = camOther;
		...
		switch (curStage)
		{
		  ...
		  case 'phillyStreets': //Weekend 1 - Darnell, Lit Up, 2Hot
				new funkin.play.stages.PhillyStreets();
			case 'phillyBlazin': // Weekend 1 - Blazin'
				new funkin.play.stages.PhillyBlazin();
		...
		if(gfVersion == null || gfVersion.length < 1)
		{
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil' | 'schoolErect':
					gfVersion = 'gf-pixel';
				case 'tank':
		...
		if (SONG.song == '2hot'
			  && !funkin.ui.freeplay.FreeplayState.scoreSongUnlocked
		    && !cpuControlled)
		{
			funkin.ui.freeplay.FreeplayState.scoreSongUnlocked = true;
			trace('Could set "scoreSongUnlocked" to ' + funkin.ui.freeplay.FreeplayState.scoreSongUnlocked);
		}
		...
		if (funkin.ui.play.pause.PauseSubState.songName != null) {
			precacheList.set(funkin.ui.play.pause.PauseSubState.songName, 'music');
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if(!Std.isOfType(songNotes[3], String)) swagNote.noteType = funkin.ui.editors.ChartingState.noteTypeList[songNotes[3]]; //Backward compatibility + compatibility with Week 7 charts
	...
	override public function update(elapsed:Float)
	{
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				if (!ClientPrefs.noSpawnFunc) callOnLuas('onSpawnNote', [notes.members.indexOf(dunceNote), dunceNote.noteData, dunceNote.noteType, dunceNote.isSustainNote]);
		...
		if (generatedMusic && !inCutscene)
		{
			if(!cpuControlled) {
				...
				&& boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 #if FLX_PITCH / FlxG.sound.music.pitch #end) * boyfriend.singDuration * singDurMult
			...
			if(startedCountdown)
			{
				...
				{
					if (!daNote.mustPress && daNote.strumTime <= Conductor.songPosition && !daNote.hitByOpponent && !daNote.ignoreNote)
					{
	...
	function openPauseMenu()
	{
		...
		if (FlxG.random.bool(0.1))
		{
			...
			cancelMusicFadeTween();
			MusicBeatState.switchState(new funkin.ui.secret.GitarooPause());
		}
		else {
			if(FlxG.sound.music != null) {
				FlxG.sound.music.pause();
			...
			openSubState(new funkin.ui.play.pause.PauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			// uncomment this if you wanna do BSOD for your mod.
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(FlxTransitionableState.skipNextTransIn) {
						funkin.ui.transition.CustomFadeTransition.nextCamera = null;
					}
					MusicBeatState.switchState(new funkin.ui.menu.StoryMenuState());

					// if ()
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						funkin.ui.menu.StoryMenuState.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = funkin.ui.menu.StoryMenuState.weekCompleted;
						FlxG.save.flush();
					}
