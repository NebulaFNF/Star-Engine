...
class CrashHandler {
	...
	private static function onUncaughtError(e:UncaughtErrorEvent):Void {
		...
		if (ClientPrefs.peOGCrash) {
			...
			CoolUtil.showPopUp(errorMessage, "Error! Star Engine v" + MainMenuState.psychEngineVersion + " (" + Main.__superCoolErrorMessagesArray[FlxG.random.int(0, Main.__superCoolErrorMessagesArray.length)] + ")");

			lime.system.System.exit(1);
		} else FlxG.switchState(new Crash());
	}
...
class Crash extends FlxState {
	...
	override public function update(elapsed:Float) if (FlxG.keys.justPressed.ANY && !clicked) {
		...
		} else if (!FlxG.keys.justPressed.F2) FlxG.switchState(new MainMenuState());
	}
}