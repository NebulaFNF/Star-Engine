...
class DebugSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('returnMemoryToFlxStringUtil', 'If checked, handles calculating the memory usage like in 0.3.0.',
			'returnMemoryToFlxStringUtil', 'bool', false);
		addOption(option);

		var option:Option = new Option('V-Slice Note Delay', 'If checked, delays the BOTPLAY and OPPONENT end animation like in V-Slice', 'vSliceNoteDelay',
			'bool', false);
		addOption(option);
